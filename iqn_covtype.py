...
def prepare_dataset(dataset):
    ...
    if np.min(Y) != -1:
        Y = 2 * Y - 1
    return X, Y
dataset = 'covtype' ## 'w8a', 'a6a', 'a9a', 'mushrooms', 'ijcnn1', 'phishing', 'splice_scale', 'svmguide3', 'german.numer_scale', 'covtype'
...
plt.plot(iqn[:75], '-', label='IQN', linewidth=2)
plt.plot(sliqn[:75], '-.', label='SLIQN', linewidth=2)
#plt.plot(iqn_sr1[:500], '--', label='iqn_sr1', linewidth=2)
plt.plot(sliqn_sr1[:75], ':', label='LISR1', linewidth=2)
plt.plot(sliqn_srk[:75], '--', label='LISR-k', linewidth=2)
